...
function discountState(p={}){const base=Number(String(p.price_value??'').replace(',','.')),sale=Number(String(p.sale_price_value??'').replace(',','.'));const active=Number.isFinite(base)&&base>0&&Number.isFinite(sale)&&sale>0&&sale<base;return {base,sale,active,percent:active?Math.max(1,Math.round((1-sale/base)*100)):0}}
...
function effectivePreviewTiers(){let items=collectPricingTiers().filter(t=>+t.quantity>1);const ds=formDiscountState();const base=ds.active?ds.sale:Number(String($('price_value').value||'').replace(',','.'));if(Number.isFinite(base)&&base>0)items=[{label:'Tekli',quantity:1,price_value:String(base),note:ds.active?'İndirimli Tekli Fiyat':'Tekli fiyat',_auto:true},...items];return items.sort((a,b)=>a.quantity-b.quantity)}
