...
function effectivePreviewTiers(){let items=collectPricingTiers();const hasSingle=items.some(t=>+t.quantity===1);const base=Number(String($('price_value').value||'').replace(',','.'));if(items.length&&!hasSingle&&Number.isFinite(base)&&base>0){items=[{label:'Tekli',quantity:1,price_value:String(base),note:'Tekli fiyat',_auto:true},...items]};return items.sort((a,b)=>a.quantity-b.quantity)}
...
async function restoreBackup(backupItem){const extra=backupItem.type==='products'?' Bu yedek yalnızca ürün verisini geri getirir; görseller mevcut haliyle kalır.':'';if(!confirm(`${formatBackupDate(backupItem.modified)} tarihli yedeğe dönülsün mü?${extra}\n\nGeri yüklemeden önce mevcut durumun tam yedeği otomatik alınacak.`))return;try{$('saveState').textContent='Yedek geri yükleniyor…';const d=await api('/api/restore',{method:'POST',body:JSON.stringify({name:backupItem.name})});toast('✅ '+d.message);$('saveState').textContent='Yedek geri yüklendi · Commit + Push';const r=await api('/api/products');products=r.products;resetForm();await refreshPublishCenter()}catch(err){$('saveState').textContent='Hata';toast(err.message,true)}}
