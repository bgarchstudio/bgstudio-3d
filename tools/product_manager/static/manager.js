...
async function load(){
  ...
  const d=await api('/api/products');products=d.products;colors=d.colors||[];$('repoPath').textContent='Repo: '+d.root;renderList();resetForm()
}
...
$('saveColors').onclick=async()=>{try{const d=await api('/api/colors/save',{method:'POST',body:JSON.stringify({colors:collectColors()})});colors=d.colors||[];renderProductColors(currentSlug?colorIdsForProduct(products.find(x=>x.slug===currentSlug)):selectedProductColorIds());$('colorsModal').hidden=true;document.body.classList.remove('modal-open');toast('✅ '+d.message);await refreshProducts(currentSlug)}catch(err){toast(err.message,true)}};
